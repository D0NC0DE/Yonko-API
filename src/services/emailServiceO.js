...
const orderTemplatePath = path.join(rootDir, 'static', 'order.ejs');
...
const sendEmail = async (type, recipientEmail, OTP = null) => {
    ...
    switch (type) {
        ...
        case 'order':
            htmlContent = await ejs.renderFile(orderTemplatePath, {
                userName: 'Welcome to Yonko!',
                orderItems: 'Thank you for signing up with Yonko. We are excited to have you on board!',
                totalAmount: '',
                deliveryAddress: '',
                deliveryDate: '',
            });
